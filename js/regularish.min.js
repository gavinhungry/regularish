...
for(var c=a.string.split("\n"),b=this.model.getMatches(),d=b.matches,e=b.groups,b="",f=0;f<d.length;f++){for(var l=d[f],g=c[f],h=0;h<l.length;h++){var k=l[h],b=b+_.escape(g.slice(0,k.from)),m=_.escape(g.slice(k.from,k.to));0<m.length&&(b+="<span>"+m+"</span>");g=g.slice(k.to)}b+=_.escape(g)+"\n"}0===b.length||"\n"===b?this.$matches.hide().html(""):this.$matches.html(b).show();Regularish.Template.get("groups",function(a){a=_.template(a,{groups:e});0===a.length||"\n"===a?this.$groups.hide().html(""):
